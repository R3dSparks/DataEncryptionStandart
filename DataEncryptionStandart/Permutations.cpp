...
{
	{0xf, 1, 8, 0xe, 6, 0xb, 3, 4, 0, 7, 2, 0xd, 0xc, 0, 5, 0xa},
...
const sbox Permutations::SBox4 =
{
	...
	{4, 3, 1, 0xb, 0xa, 0xd, 7, 8, 0xf, 9, 0xc, 5, 6, 3, 0, 0xe},
...
{
	{0xd, 2, 8, 4, 6, 0xf, 0xb, 1, 0xa, 8, 3, 0xe, 5, 0, 0xc, 7},
...
uint64_t Permutations::Permutate(uint64_t _value, int _output_length, const perm _permutationTable)
{
	uint64_t perm_value = 0;

	for (int i = 0; i < _output_length; i++)
	{
		setBit(perm_value, i, _value & ((uint64_t)0x1 << _permutationTable[i]));
	}

	return perm_value;
}

int Permutations::SBoxSubstitution(uint64_t _value)
{
	int result = 0;
	int s_round = 0;
